...
public class SilencerPowerupRenderer extends EntityRenderer<SilencerPowerupEntity, EntityRenderState> {

    private static final Identifier SILENCER_LOCATION = Identifier.fromNamespaceAndPath(Quake2Weapons.MODID,"textures/entity/itempickups/silencer_powerup.png");
    private final SilencerPowerupModel model;
    ...
    public void submit(EntityRenderState state, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraState) {
        ...
        nodeCollector.submitModel(this.model, state, poseStack, this.model.renderType(SILENCER_LOCATION), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
