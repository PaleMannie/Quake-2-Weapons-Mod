...
public class RebreatherPickupRenderer extends EntityRenderer<RebreatherPickupEntity, EntityRenderState> {

    private static final Identifier REBREATHERPICKUP_LOCATION = Identifier.fromNamespaceAndPath(Quake2Weapons.MODID,"textures/entity/itempickups/rebreather_pickup.png");
    private final RebreatherPickupModel model;
    ...
    public void submit(EntityRenderState state, PoseStack poseStack, SubmitNodeCollector nodeCollector, CameraRenderState cameraState) {
        ...
        nodeCollector.submitModel(this.model, state, poseStack, this.model.renderType(REBREATHERPICKUP_LOCATION), state.lightCoords, OverlayTexture.NO_OVERLAY, state.outlineColor, null);
